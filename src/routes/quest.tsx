...
function QuestPage() {
  const { configured, profile, loot, claims, loading, error, signUp, signIn, submitProof } = useLootDrop();
  ...
  if (!profile)
    return (
      <div className="space-y-4 border-2 border-outline bg-card p-4 pixel-shadow">
        ...
        <p className="text-sm text-muted-foreground">Your username and password keep your XP, level and finds safe.</p>
        <input value={username} onChange={(event) => setUsername(event.target.value)} maxLength={24} placeholder="Username" autoComplete="username" className="w-full border-2 border-outline bg-background px-3 py-2 text-sm" />
        <input value={password} type="password" onChange={(event) => setPassword(event.target.value)} placeholder="Password" autoComplete={mode === "signup" ? "new-password" : "current-password"} className="w-full border-2 border-outline bg-background px-3 py-2 text-sm" />
        <button
          disabled={busy}
          onClick={async () => {
            setBusy(true); setAuthError(null);
            try { await (mode === "signup" ? signUp(username, password) : signIn(username, password)); }
            catch (cause) { setAuthError(cause instanceof Error ? cause.message : "Could not sign in."); }
        ...
        <button onClick={() => { setMode(mode === "signup" ? "signin" : "signup"); setAuthError(null); }} className="w-full font-pixel text-[8px] text-muted-foreground underline">
